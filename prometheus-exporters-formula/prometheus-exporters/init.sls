...
{% set is_transactional = grains['transactional'] %}
  ...
  {% if proxy_enabled %}
    {% if is_transactional %}
      test.fail_without_changes:
        - name: "Exporter proxy is not supported on transactional systems!"
    {% else %}
      pkg.installed:
...
{% if apache_exporter_enabled %}
  {% if is_transactional %}
      test.fail_without_changes:
        - name: "Apache exporter is not supported on transactional systems!"
  {% else %}
...
{% if postgres_exporter_enabled %} 
  {% if is_transactional %}
      test.fail_without_changes:
        - name: "PostgreSQL exporter is not supported on transactional systems!"
  {% else %}
...
{% if postgres_exporter_enabled and proxy_enabled and proxy_supported and not is_transactional %}
